...
describe('point', () => {
    const v1 = point(frame, 10, 20, 30);
    const v2 = vector(1, 2, 3);
    test("default", () => expect(point(frame)).toEqual(point(frame, 0, 0, 0)));
    test('add', () => expect(add(v1, v2)).toEqual(point(frame, 11, 22, 33)));
    test('addType', () => expect(isPoint(add(v1, v2))).toBe(true));
    test('sub', () => expect(sub(v1, v2)).toEqual(point(frame, 9, 18, 27)));
    test('subType', () => expect(isPoint(sub(v1, v2))).toBe(true));
    // @ts-expect-error
    test('mul', () => expect(() => mul(v1, 2, 3)).toThrowError());
    test("equal", () => expect(equal(v1, point(frame, 10, 20, 30))).toBe(true));
    // @ts-expect-error
    test("not equal", () => expect(equal(v1, v2)).toBe(false));
});
...
describe('orientation', () => {
    ...
    test('mul', () => expect(() => mul(v1, 2, 3)).toThrowError());
    test("equal 0", () =>
        expect(equal(v1, orientation(frame)))
            .toBe(true));
    test("coerce", () =>
        expect(Orientation.coerce(frame, v2))
            .toEqual(Orientation.fromEuler(frame, Math.PI/2, 0, 0)));
    test("coerceX", () =>
        expect(Orientation.coerce(frame, Rotation.fromEuler(Math.PI/2, 0, 0)))
            .toEqual(Orientation.fromEuler(frame, Math.PI/2, 0, 0)));
    test("equal", () =>
